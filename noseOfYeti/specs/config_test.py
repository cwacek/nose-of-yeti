...
class Test_Default(object):
    ...
    def test_stringify_returns_value(self):
        ...
        if six.PY3:
            ...
            six.text_type(Default("blah")) | should | equal_to("blah")
            six.text_type(Default(1)) | should | equal_to("1")
            six.binary_type(Default("blah")) | should | equal_to("blah")
            six.binary_type(Default(1)) | should | equal_to("1")

            six.text_type(Default(Special())) | should | equal_to("things")
            six.binary_type(Default(Special())) | should | equal_to("stuff")
...
class Test_ConfigSetup(object):
    ...
    def test_it_gets_values_from_config_file(self):
        ...
        with a_temp_file(contents) as filename:
            ...
            config._util.values | should | equal_to(
                {"config_file": filename, u"a": 1, u"b": 2})
    ...
    def test_it_overwrites_config_file_with_options(self):
        ...
        with a_temp_file(contents) as filename:
            ...
            config._util.values | should | equal_to(
                {"config_file": filename, "a": 4, u"b": 2})
...
class Test_ConfigAttributes(object):
    ...
    def test_config_gets_attributes_from_util(self):
        ...
        util = fudge.Fake("util").expects(
            "find_value").with_args(key).returns(val)
        config = Config()
        config._util = util
